...
Python is **not** a runtime or build dependency. The metadata in
...
rebuilt by `npm run build:search-index` (pure-JS, part of `npm run build`
and the Docker build). See [Data pipeline](#data-pipeline).
...
- Node.js 20+ and npm
- Nothing else. Python is no longer required.
...
`public/issues.json` and `public/issues_full.json` are committed to git
(hand-curated) and are **not** regenerated by the build. The gitignored
`public/search_index.json` is rebuilt by `npm run build:search-index`
(which is invoked as part of `npm run build`). Vite's `publicDir` copy
moves whatever is in `public/` into `dist/` verbatim.
...
| `npm run build` | Full production build: WebP covers → `build:search-index` → `tsc -b` → `vite build`. The search index is regenerated unconditionally; Vite's `publicDir` copy picks it up. |
...
| `npm run build:search-index` | Pure-JS rebuild of the gitignored `public/search_index.json` (and `.gz`) from `public/issues_full.json` and the `issues/` tree. Idempotent (mtime-skip) when the output is newer than every input. |
...
│   │   ├── articleDisplay.ts              # Re-export of cleanArticleTitle (React-friendly
│   │   │                                 #   displayTitle wrapper)
│   │   ├── cleanArticleTitle.ts           # Pure subtitle-cleaning rule — single source of
│   │   │                                 #   truth, shared with the build-time index generator
...
├── scripts/
│   ├── build-search-index.mjs             # Pure-JS: strip HTML, write public/search_index.json(.gz)
│   ├── build-search-index.test.mjs        # node:test smoke tests for the index builder
│   ├── convert-covers-to-webp.js          # wraps system cwebp (idempotent)
│   └── normalize-issue-fonts.js           # one-time 127+ font filename aliasing
...
On mount, the provider fetches `/issues_full.json` (from `CONFIG.DATA_URL`) and computes the year buckets. The `selectedIssue` setter is the only piece of state that escapes the provider's "data" responsibility — it's intentionally colocated with the archive because opening a cover is an archive concern, not a UI concern.
...
The metadata in `public/issues.json` and `public/issues_full.json` is
**committed to git and hand-curated**. The archive is finite, so we do
not regenerate it on every build. The runtime container is a pure nginx
serve — no Python, no in-container metadata extraction.

The gitignored `public/search_index.json` (and its `.gz` sibling) is
regenerated from `public/issues_full.json` and the `issues/` tree by
`scripts/build-search-index.mjs`, a pure-JS port of the original Python
extractor. The script is invoked by `npm run build:search-index` and
is part of `npm run build`, so the Docker image always ships a fresh
index that matches the committed metadata.

**Do not add a script that regenerates `issues.json` or
`issues_full.json`.** Those files have 99 hand-corrections for
department-style articles in issues 50-63 and any number of one-off
fixes; running a fresh extraction would silently destroy them.
...
`cleanArticleTitle(subject, title)` in `src/utils/cleanArticleTitle.ts` is the rule:
...
and determining the actual subject.

### Search index details

The search index in `public/search_index.json` powers full-text mode. It is:

- **Gitignored** — the runtime image is built from a fresh copy.
- Built by `npm run build:search-index`, which is invoked as part of
  `npm run build` and the Docker build. The script is
  `scripts/build-search-index.mjs`. Once built, the file is committed to the
  runtime image (it lives in `public/`, which Vite copies into `dist/`).
- Generated by stripping HTML tags (the `extractText` helper in
  `scripts/build-search-index.mjs`, which discards `<script>` / `<style>` /
  `<noscript>` content and condenses whitespace) and writing one record
  per `(issue, articleIndex)` pair.
