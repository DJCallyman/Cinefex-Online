...
| Runtime image | **nginx 1.27 (alpine)** + **python 3** for the runtime metadata regen |
| CI | **GitHub Actions** (`docker/build-push-action` → GHCR) |

Python 3 is the only data-pipeline dependency. It is needed to regenerate `public/issues_full.json` and the search index from `issues/{N}/manifest.xml` files.
...
- Node.js 20+ and npm
- Python 3.8+ (only required when adding or regenerating issue metadata, or for the runtime `create_json.py` regen inside the container)
...
2. `npm run search:index` — runs `python3 create_json.py` to regenerate `public/issues_full.json`, `public/issues.json`, and `public/search_index.json` (+ `.gz`).
3. `tsc -b` — type-check the project.
4. `vite build` — emit the production bundle into `dist/`.

Vite's `publicDir` copy takes care of moving the freshly-generated `issues_full.json`, `issues.json`, and `search_index.json` into `dist/` so the app's runtime fetch of `/issues_full.json` resolves correctly in both dev and production.
...
| `npm run build` | Full production build: WebP covers → search index → `tsc` → `vite build`. |
...
| `npm run search:index` | Run `python3 create_json.py` standalone (rebuild JSON + search index only). |
...
│   ├── issues_full.json       # Archive metadata — generated by create_json.py, gitignored
│   ├── issues.json            # Issues 1–126 only — generated, gitignored
...
The search bar is debounced with `CONFIG.DEBOUNCE_MS` (300 ms). Escape clears the input and blurs it.
...
4. Each article HTML file contains `<meta name="Film">` and `<meta name="Title">` (or Dublin Core equivalents) — these are extracted by `create_json.py` and used for the cover card's title, the modal's article list, and the full-text search index.
5. `npm run search:index` (or `python3 create_json.py` directly) runs from the repo root and:
   - Walks every `issues/{N}/manifest.xml` (1 through 169), fixes unescaped ampersands, parses the `<article>` / `<readingView>` / `<imageGallery>` sections, and uses the `<article>` filenames (`N.ArchivalView.html`) to infer the matching `N.ReadingView.html`.
   - Falls back to the manifest's `<readingView name=…>` mapping for legacy files.
   - Looks up `<imageGallery name=…>` with a case-insensitive fallback (to handle the rare "The Finest hours" vs "The Finest Hours" capitalisation mismatches).
   - Skips issues with no usable HTML articles on disk (cover-only entries).
   - Writes `public/issues_full.json` (all issues, sorted by issue number), `public/issues.json` (issues 1–126 only, for the legacy `check_article_names.py` script), and the full-text search index `public/search_index.json` (+ `.gz`).
   - Anchors to the script's own directory so the `issues/` and `public/` paths work from any CWD.
6. Vite's build copies `public/` into `dist/`, so the app's runtime fetch of `/issues_full.json` resolves correctly in both dev and production.
7. The Docker image's runtime entrypoint (`docker/entrypoint.sh`) **re-runs** `create_json.py` on every container start against the bind-mounted `ISSUES_DIR`, then atomically replaces the bundle's pre-baked `issues_full.json` / `issues.json` / `search_index.json` / `search_index.json.gz` with the freshly-generated ones, then symlinks `$ISSUES_DIR` over the empty placeholder `dist/issues/`. This means dropping a new issue into the share is enough — no image rebuild needed.
8. (Optional, recommended.) Run `python check_article_names.py` to validate that the article names in `issues.json` still match the actual `<Title>` / `articleTitle` elements in the reading-view HTML files.
...
- Generated by stripping HTML tags (`TextExtractor` in `create_json.py`, which discards `<script>` / `<style>` / `<noscript>` content and condenses whitespace) and writing one record per `(issue, articleIndex)` pair.
- Per-article body text is truncated to `max_chars_per_doc` (24 kB by default) to keep the shipped payload small. The vast majority of articles fit completely; only the longest 127+ features get clipped, and the text near the start is usually the most search-relevant anyway (article openings, byline, intro).
- Loaded **lazily** by the client (`useSearchIndex`), and **only** when the user switches to full-text mode. In the default title mode, the index is never fetched, so the wire payload stays at zero.
- A `.gz` sibling is also written, and the nginx config enables `gzip_static` for `application/json` so the file ships compressed over the wire (~6 MB gzipped for ~766 documents).
...
- **Builder stage** — `node:20-bookworm`. Installs Python 3 + ca-certificates, runs `npm ci --include=dev`, then strips the dev-only `public/covers` and `public/issues` symlinks (which point at `../covers` and `../issues` on a Mac but not in CI), replaces them with empty placeholders, and runs `tsc -b && vite build`. The `--include=dev` flag is required so `vite` and `@tailwindcss/postcss` are present during the build.
- **Runtime stage** — `nginx:1.27-alpine`. Installs `python3` and upgrades `libexpat` (Alpine's `nginx:1.27-alpine` ships `libexpat` 2.7.0, but Python's `pyexpat` needs `libexpat` ≥ 2.7.5; without the upgrade, `create_json.py` fails to parse the manifest XMLs and the app boots with zero issues — visible as "no covers" in the browser). Copies the builder's `dist/`, the baked `covers/`, `create_json.py`, the `docker/nginx.conf`, and the `docker/entrypoint.sh` (renamed to `40-cinefex.sh` so it chains onto the base image's own `/docker-entrypoint.d/` scripts before nginx starts).
- **Healthcheck** — `python3 -c "import urllib.request,sys; sys.exit(0 if urllib.request.urlopen('http://localhost/issues_full.json',timeout=3).status==200 else 1)"`. The same check is wired into `docker-compose.yml`.

The entrypoint (`docker/entrypoint.sh`):

1. Waits up to 30 s for `ISSUES_DIR` to be bind-mounted and non-empty (unraid occasionally races the share mount).
2. Re-runs `create_json.py` against the freshly-mounted issues tree.
3. Copies the freshly-generated JSON files into nginx's docroot.
4. `rm -rf`s the docroot's empty `issues/` placeholder and `ln -sfn`s the bind-mounted issues tree over it. (`$DOCROOT/covers` is already populated at build time and doesn't need a runtime symlink.)
5. Exits 0 so the base image's own `/docker-entrypoint.sh` starts nginx.
...
unraid pulls `:latest` on container restart. To pick up the latest build, stop and start the container (right-click → Restart works too). The issues share is bind-mounted, so the new image sees your existing issues immediately — **no image rebuild required to add issues**.
